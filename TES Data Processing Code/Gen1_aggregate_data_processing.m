% NOTE: This is for FSS Gen 1.0 ONLY! To get table for FSS Gen 2.0, use 
...
% follow this format for the general directories:
    % cd 'C:\ ....... \FSS Design Validation Study\FSS Gen 1.0'
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 1.0\Code"));
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 1.0\Data"));
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 1.0\Figures"));
cd ''; % main folder
addpath(genpath("")); % folder with code
addpath(genpath("")); % folder with data
addpath(genpath("")); % folder for figures

% directory for pooling data
% follow this format for the pooled data directory:
    % file_dir = 'C:\ ....... \FSS Design Validation Study\FSS Gen 1.0\Data\Processed_Data_Study1\';
file_dir = '';
...
trials = [4,4,4,1,1,4,3,3]; % best trials for each subject
...
for i_subject = 1:n_sub % loop through all subjects
    ...
    % Open Subject data
    data_name = strcat(file_dir,'\', subjects{i_subject}, '_MASTER_DATA_TRIAL_',num2str(i_trial),'.mat');
    load(data_name);
...
% generate final table of Gen 1.0 FSS data
final_table_fss_1 = array2table(table_array);
% add row labels
final_table_fss_1.Properties.RowNames = {'Fx(N) AVG','Fx(N) SE',...
                                         'Fy(N) AVG','Fy(N) SE',...
                                         'Fz(N) AVG', 'Fz(N) SE',...
                                         'Mx(Nm) AVG', 'Mx(Nm) SE',... 
                                         'My(Nm) AVG', 'My(Nm) SE',...
...
% add column labels
final_table_fss_1.Properties.VariableNames = {'[ABU] RMSE', '[ABU] MAX', '[ABU] MIN',...
                                              '[MWCU] RMSE', '[MWCU] MAX', '[MWCU] MIN'};

% export final table as an Excel spreadsheet                                          
table_name = strcat(table_dir, 'FSS_Gen_1.0_table.xls');
writetable(final_table_fss_1, table_name);
disp(final_table_fss_1)
