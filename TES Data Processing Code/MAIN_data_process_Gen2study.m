% NOTE: This is for FSS Gen 2.0 ONLY! To get master data and plots for 
% FSS Gen 1.0, use MAIN_data_process_Gen1study.m
    % This code processes the FSS, QTM (Qualisys motion capture), VN IMU, and
    % AMTI force plate data for the Gen 2.0 FSS design validation study
...
% follow this format for the general directories:
    % cd 'C:\ ....... \FSS Design Validation Study\FSS Gen 2.0'
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 2.0\Code"));
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 2.0\Data"));
    % addpath(genpath("C:\ ....... \FSS Design Validation Study\FSS Gen 2.0\Figures"));
cd ''; % main folder
addpath(genpath("")); % folder with code
addpath(genpath("")); % folder with data
addpath(genpath("")); % folder for figures
    
% directory for loading data files
% follow this format for data file directory:
    % file_dir = 'C:\ ....... \FSS Design Validation Study\FSS Gen 2.0\Data\';
file_dir = '';

% directory for saving new data (your choice)
save_dir = '';


%% Define subjects 
subjects = {'S8'}; % choose which subject's data to process, use a capitalized "S"
raw_plot_status = 0; % 1 - PLOT, 0 - NO PLOT
plot_status = 1; % 1 - PLOT, 0 - NO PLOT
...
[i_start_all, i_amti_start, i_qtm_start, i_end_idx_array,...
 crop_status_array, i_crop_s_array, i_crop_e_array] = Gen2_get_offsets(subjects{1});
...
% Note: this applies only for the Gen 2.0 FSS study    
...
% s9                2
        ...
        %% Load FSS data file
        fss_filename = strcat(file_dir, i_subject{1}, '\', i_subject{1}, '_TRIAL', num2str(i_trial), '_FSS.txt');
        raw_fss_data = readmatrix(fss_filename);
        fss_lc_data_ = raw_fss_data(:, 2:7); % FSS load cell data
        
        % process the VN IMU data
        vn_data_table = process_imu_vn_data(raw_fss_data(:,8:10));
        vn_data__ = table2array(vn_data_table);
        vn_data_ = vn_data__(1:end,:); % processed VN IMU data
        
        idx_master_pure = [1:length(vn_data_)]';
        
          
        %% Load AMTI data file
        amti_filename = strcat(file_dir, i_subject{1}, '\',i_subject{1}, '_TRIAL', num2str(i_trial), '_AMTI.txt');       
        ...
        qtm_filename = strcat(file_dir, i_subject{1}, '\',i_subject{1}, '_TRIAL', num2str(i_trial), '_QTM.mat');
        ...
        %% Calibration matrix
        m = fss_lc_data_';
        load(strcat(file_dir,'jacobian_matrix_study2.mat'))
        W_pred = (- E) * m;
        W_pred_t_ = W_pred';
        
              
        %% Start at roughly same index 
        % use offsets that were found earlier with Gen2_get_offsets.m to
        % align all data in the time axis
        if i_end_idx_array{i_trial} == 1
            i_end_idx = length(vn_data_) - i_start_all(i_trial);
        else
        ...
        idx_master_pure_ = idx_master_pure(i_start_all(i_trial):i_start_all(i_trial)+i_end_idx,:);
        idx_master_amti_ = idx_master_amti(i_amti_start(i_trial) + i_start_all(i_trial):end,:);
        idx_master_qtm_ = idx_master_qtm(i_qtm_start(i_trial) + i_start_all(i_trial):end,:);
        ...
        if raw_plot_status == 1
            ...
            figure()
            sgtitle('Raw FSS Data for Gen 2.0')
            for i_subplot = 1:6
            ...
            figure()
            sgtitle('Raw IMU Data for Gen 2.0')
            for i_subplot = 1:3
        ...
        idx_delay_qtm = round(mean(idx_delay_qtm_array));
        ...
        if plot_status == 1
            ...
            figure()
            sgtitle('Processed FSS Data for Gen 2.0')
            for i_subplot = 1:8
        ...
        %% Plot IMU data
            ...
            figure()
            sgtitle('Processed IMU Data for Gen 2.0')
            for i_subplot = 1:3
                subplot(3,1,i_subplot)
        ...
        if save_status
            ...
            save_name = strcat(save_dir, 'GEN2FSS_', i_subject{1}, '_MASTER_DATA_TRIAL_', num2str(i_trial), '.mat');
